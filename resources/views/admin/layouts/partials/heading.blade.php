    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown no-arrow mx-1">
            <a class="nav-link dropdown-toggle" href="#" id="messagesDropdown" role="button" data-toggle="dropdown"
                ...
                @if (auth()->user()->doctor)
                    <span
                        class="badge badge-danger badge-counter">{{ auth()->user()->doctor->unreadNotifications->count() }}</span>
                @else
            ...
            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
                ...
                @if (auth()->user()->doctor)
                    @foreach (auth()->user()->doctor->unreadnotifications as $notification)
                        @if ($notification->read_at === null)
                            <a class="dropdown-item d-flex align-items-center" href="#">
                                ...
                                <div class="font-weight-bold">
                                    <div class="text">{{ $notification->data['message'] }}</div>
                                    <div class="small text-gray-500">{{ $notification->data['type'] }} ·
                ...
                @if (auth()->user()->hasRole('super-admin'))
                    ...
                    {{-- <form action="{{ route('notifications.markAllAsRead') }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-sm  btn-outline-primary" data-toggle="tooltip"
                            data-placement="top" title="Mark all as read">Mark all as Read</button>
                    </form> --}}
                    {{-- <a class="dropdown-item text-center small text-gray-500" href="#">Read More Messages</a> --}}
                @endif
                @foreach (auth()->user()->unreadNotifications as $notification)
                    @if ($notification->read_at === null)
                        <form action="{{ route('notifications.markAllAsRead') }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-sm text-align-center" data-toggle="tooltip"
                                data-placement="top" title="Mark all as read">Mark all as Read</button>
                        </form>
                    @endif
                @endforeach
